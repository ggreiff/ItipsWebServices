...
namespace OppmHelper.OppmApi
{
    public class SePortfolioSecurity
    {
        ...
        /// <summary>
        /// Logouts the specified security token.
        /// </summary>
        /// <param name="securityToken">The security token.</param>
        /// <returns>Boolean.</returns>
        public Boolean Logout(String securityToken)
        {
            if (securityToken.IsNullOrEmpty()) return true;
            try
            {
                PortfoliosSecurityWs.ReleaseSecurityToken(securityToken);
                return true;
            }
            catch (Exception ex)
            {
                PsLogger.Warn(string.Format("Unexcpected Logout Error: \n{0}\n", ex.Message));
                return false;
            }
        }
