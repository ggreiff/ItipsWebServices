...
namespace OppmHelper.OppmApi
{
    public class SePortfolioSubItem
    {
        ...
        {
            psPortfoliosSubItemUpdateStatus[] retVal;
            var asOfString = sAsOf.ToString("MM/dd/yyyy");
            if (asOfString.Equals(DateTime.Now.ToString("MM/dd/yyyy"))) asOfString = String.Empty;
            try
            {
                retVal = PsSubItem.SyncSubItemsAsOf(sCommonIdCategory, sId, sSubItemType, lSubItemTypeId,
                                                        arSubItemList.ToArray(), asOfString, stopOnAnyError);
            }
            catch (Exception ex)
            {
                if (PsLogger != null)
